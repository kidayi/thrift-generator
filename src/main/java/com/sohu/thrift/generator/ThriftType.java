...
public class ThriftType implements Cloneable {
	...
	public static ThriftType fromJavaType(Class<?> clazz) {
		...
		if(clazz == short[].class || clazz == Short[].class) {
			...
			generic.setValue("list");
			generic.addGeneric(I16);
			return generic;
		}
		...
		if(clazz == int[].class || clazz == int[].class) {
			Generic generic = new Generic();
			generic.setValue("list");
			generic.addGeneric(I32);
			return generic;
		}
		...
		if(!clazz.getName().startsWith("java.lang")) {
			...
			thriftType.setValue(clazz.getSimpleName()+Constants.end_str);
			thriftType.setJavaClass(clazz);
			return thriftType;
		}
